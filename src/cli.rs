    ...
    let diagnostics = Diagnostics {
        ...
        exec_order: {
            ...
            for &rank in &rank_nums {
                ...
                let Ok(content) = std::fs::read_to_string(&cd_path) else {
                    continue;
                };
                ...
                // Find latest artifact that contains execution order information.
                let mut best: Option<(u64, String)> = None; // (number, url)
                for (_cid, entry) in map.iter() {
                    let Some(arts) = entry.get("artifacts").and_then(|x| x.as_array()) else {
                        continue;
                    };
                    for a in arts {
                        let name = a.get("name").and_then(|x| x.as_str()).unwrap_or("");
                        // Match: "graph_execution.json" or "graph_execution_*.json"
                        let is_graph_exec = name == "graph_execution.json"
                            || (name.starts_with("graph_execution") && name.ends_with(".json"));
                        if is_graph_exec {
                            if let (Some(num), Some(url)) = (
                                a.get("number").and_then(|x| x.as_u64()),
                                a.get("url").and_then(|x| x.as_str()),
                            ) {
                                match best {
                                    Some((bn, _)) if bn >= num => {}
                ...
                if let Some((_n, url)) = best {
                    ...
                    if let Ok(payload) = std::fs::read_to_string(path) {
                        if let Ok(order_raw) = parse_graph_execution_order(&payload) {
                            // Normalize compile_id format to match compile_directory keys (e.g., "[0/1]")
                            let order: Vec<String> =
                                order_raw.into_iter().map(|s| format!("[{}]", s)).collect();
            ...
            if exec_orders.len() < 2 {
                ...
                let mut dir_to_compile_id_per_rank: FxHashMap<u32, FxHashMap<String, String>> =
                    FxHashMap::default();
                for &rank in &rank_nums {
                    let rank_dir = out_path.join(format!("rank_{rank}"));
                    let cd_path = rank_dir.join("compile_directory.json");
                        ...
                        if let Ok(v) = serde_json::from_str::<serde_json::Value>(&s) {
                            if let Some(obj) = v.as_object() {
                                ...
                                for (cid, entry) in obj.iter() {
                                    if let Some(arts) =
                                        entry.get("artifacts").and_then(|x| x.as_array())
                                    {
                                        for a in arts {
                ...
                for &rank in &rank_nums {
                    let rank_dir = out_path.join(format!("rank_{rank}"));
                    let cd_path = rank_dir.join("compile_directory.json");
                    let Ok(s) = std::fs::read_to_string(cd_path) else {
                        continue;
                    };
                    ...
                    let Some(dir2cid) = dir_to_compile_id_per_rank.get(&rank) else {
                        continue;
                    };
                ...
                // 4) Evaluate
                let report =
                    analyze_execution_order(&exec_orders, &collective_by_graph, &cache_status);

                // Summaries
                use std::collections::HashSet;
                let order_differs = report.by_index.iter().any(|row| {
                    let uniq: HashSet<&str> = row.by_rank.values().map(|s| s.as_str()).collect();
                    uniq.len() > 1
                });
